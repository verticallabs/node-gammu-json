...
exports.prototype = {
    ...
    _transmit_messages: function (_callback) {
      ...
      self._subprocess(this._gammuJsonPath, args, function (_err, _rv) {
    ...
    _receive_messages: function (_callback) {
      ...
      self._subprocess(this._gammuJsonPath, [ 'retrieve' ], function (_err, _rv) {
     ...
     /* Segment cache:
      ...
      self._delete_batch_size = (
        options.delete_batch_size || 1024
			...
			self._gammuRcPath = (
				options.gammuRcPath || null
			);

			self._gammuJsonPath = (
				options.gammuJsonPath || null
			);
    ...
    _subprocess: function (_path, _argv, _options, _callback) {
			if(this._gammuRcPath) {
				_argv.unshift(this._gammuRcPath);
				_argv.unshift('-c');
			}
      ...
      subprocess.stderr.on('data', function (_buffer) {
				errors += _buffer.toString(); 
      });
